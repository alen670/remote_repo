...
WITH RECURSIVE leader_path AS (
    -- 先锚定所有员工（包括没有上级的）
    SELECT id, name, managerid, name AS path 
    FROM emp 
    WHERE managerid IS NULL 
    UNION ALL 
    -- 递归部分：将员工连接到其上级的路径上
    SELECT e.id, e.name, e.managerid, CONCAT(lp.path, ' -> ', e.name) 
    FROM emp e 
    INNER JOIN leader_path lp ON e.managerid = lp.id
)
SELECT * FROM leader_path
ORDER BY id;
```
字段数相同,字段类型要一致
