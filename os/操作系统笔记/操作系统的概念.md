...
class BankerAlgorithm:
    def __init__(self, n, m, available, max_demand, allocation):
        """
        初始化银行家算法
        n: 进程数量
        m: 资源种类数量
    ...
    def request_resources(self, process_id, request):
        ...
        # 步骤4: 执行安全性检查
        if self._safety_check(temp_available, temp_allocation, temp_need):
            # 安全，正式分配资源
            self.available = temp_available
            self.allocation = temp_allocation
            self.need = temp_need
            print(f"资源分配成功！系统处于安全状态")
            return True
        else:
            print(f"资源分配会导致系统进入不安全状态，分配被拒绝")
            return False
    
    def _safety_check(self, available, allocation, need):
        """
        安全性检查算法
        """
        work = available.copy()
        ...
        while len(safe_sequence) < self.n:
            ...
            for i in range(self.n):
                if not finish[i] and all(need_copy[i] <= work):
                    # 找到可以满足的进程
                    work += allocation_copy[i]
                    finish[i] = True
                    safe_sequence.append(i)
                    found = True
                    print(f"找到安全进程P{i}, 工作向量更新为: {work}")
                    break
        ...
        if len(safe_sequence) == self.n:
            print(f"安全序列: {['P' + str(i) for i in safe_sequence]}")
            return True
        else:
            print("未找到安全序列")
            return False
...
def test_banker_algorithm():
    ...
    # 创建银行家算法实例
    banker = BankerAlgorithm(n, m, available, max_demand, allocation)
    
    # 显示初始状态
    banker.display_status()
    
    # 测试安全序列检查
    print("\n=== 初始状态安全性检查 ===")
    banker._safety_check(banker.available, banker.allocation, banker.need)
    ...
    # 示例2: 超过最大需求的请求
    print("\n--- 示例2: P0请求[8, 0, 0] (超过最大需求) ---")
    banker.request_resources(0, [8, 0, 0])
    
    # 示例3: 资源不足的请求
    print("\n--- 示例3: P0请求[3, 3, 0] (资源不足) ---")
    banker.request_resources(0, [3, 3, 0])
...
改进型CLOCK算法的设计初衷有**两个**，并且它们有明确的**优先级顺序**：

1.  **首要目标（更高优先级）：** 尽可能保留**最近被访问过**的页面，因为这些页面在不久的将来被再次访问的可能性更大（局部性原理）。这是通过“访问位”来判定的。
2.  **次要目标（较低优先级）：** 在满足首要目标的前提下，**优先淘汰未被修改过的页面**，以**减少耗时的I/O写回操作**。这是通过“修改位”来判定的。

所以，优先级顺序是：**访问位 > 修改位**。

- 2. 算法的四轮扫描是如何体现这个优先级的？

算法通过四轮扫描，将页面分成了四个优先级类别，从低到高依次寻找淘汰对象：

*   **第一优先级（最好淘汰）：(0,0)**
    *   **状态**：最近没被访问 + 没被修改。
    *   **为什么最好**？淘汰它既不会牺牲一个“活跃”页面（因为访问位=0，说明它近期不活跃），也**不需要进行I/O操作**（因为修改位=0）。这是最“干净”的淘汰。
    *   **对应轮次**：**第一轮**就找它。

*   **第二优先级（次好淘汰）：(0,1)**
    *   **状态**：最近没被访问 + 但被修改过。
    *   **为什么是第二好**？虽然淘汰它需要一次I/O写回操作（缺点），但它毕竟是一个“不活跃”的页面（访问位=0），牺牲掉它是相对安全的。**用一次I/O操作来换掉一个不活跃的页面，是值得的。**
    *   **对应轮次**：**第二轮**找它。

---**到这里请注意**：前两轮的目标非常明确，就是**专门处理所有“不活跃”（访问位=0）的页面**。在处理完所有不活跃的页面之前，**绝对不会**去碰任何“活跃”（访问位=1）的页面。

*   **第三优先级（尽量避免淘汰）：(1,0)**
*   **第四优先级（最应避免淘汰）：(1,1)**
    *   **状态**：这两个都是“活跃”页面（访问位=1）。
    *   **为什么尽量避免**？因为它们最近被访问过，根据局部性原理，它们很快再次被访问的可能性很大。如果淘汰它们，很可能很快又会产生缺页中断，性能低下。**与保留一个活跃页面带来的性能提升相比，是否发生I/O操作是次要考虑的问题。**
    *   **对应轮次**：如果前两轮都失败了（说明当前循环队列里连一个“不活跃”的页面都找不到，所有页面都是活跃的），算法才会进入第三、四轮来处理这些活跃页面。在第三轮开始前，算法会**将所有扫描过的页面的访问位重置为0**，强行将它们“降级”为不活跃状态，然后再从(0,0)和(0,1)里找。

- 3. 回答您的核心疑问：为什么第二轮是(0,1)而不是(1,0)？

现在我们就可以直接回答您的问题了：

如果将第二轮设置为淘汰(1,0)，那就意味着：
**算法选择淘汰一个“最近很活跃，但很干净”的页面，而保留一个“最近不活跃，但很脏”的页面。**

这**严重违背**了“访问位优先级高于修改位”的初衷。因为您为了节省一次I/O操作，却付出了一代价：**牺牲了一个活跃页面**。这很可能导致刚被淘汰的这个活跃页面马上又需要被调入，引发不必要的缺页中断，性能损失更大。

而算法的实际设计（第二轮淘汰(0,1)）是：
**算法选择淘汰一个“最近不活跃，但很脏”的页面，而保留一个“最近很活跃，但很干净”的页面。**
...
内存映射文件除了能方便访问修改硬盘文件,还可以实现文件的共享
