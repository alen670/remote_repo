...
struct Kernel_Thread* Get_Next_Runnable(void)
{
    ...
    if (g_schedulingPolicy == ROUND_ROBIN) {
        ...
        best = Find_Highest_Priority(&s_feedbackQueue);
    }
...
// 定义全局变量 g_schedulingPolicy
int g_schedulingPolicy = ROUND_ROBIN;

// 定义 s_feedbackQueue
struct Thread_Queue s_feedbackQueue;

/*
 * Find the best (highest priority) thread in given
 * thread queue.  Returns null if queue is empty.
 */
struct Kernel_Thread* Find_Highest_Priority(struct Thread_Queue* queue) {
    // 示例实现：返回队列中的第一个线程
    return queue->head;
}
